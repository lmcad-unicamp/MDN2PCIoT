...
void Build2DSubsamplingLayer(Graph Source_graph, int conv_starts, int width_input, int height_input, int layer);
...
int main(int argc, char *argv[]) {
	...
	// subsampling S2
	Build2DSubsamplingLayer(Source_graph, initialPos[1], wi[1], hi[1], 2); 

	// conv C3
	BuildGraph2DConvolutionalLayer(Source_graph, initialPos[3], wi[2], hi[2], initialPos[2], 3);

	// subsampling S4
	Build2DSubsamplingLayer(Source_graph, initialPos[3], wi[3], hi[3], 4);

	// conv C5
	BuildGraph2DFullyConnectedLayer2Dinput(Source_graph, initialPos[5], wi[4], hi[4], initialPos[4], 5);	

	// FC6
	BuildGraph2DFullyConnectedLayer(Source_graph, di[5], di[6], initialPos[5], initialPos[6]);

	// Output
	BuildGraph2DFullyConnectedLayer(Source_graph, di[6], di[7], initialPos[6], initialPos[7]);
...
int WriteInputFile(Graph Source_graph,
	...
	strcpy(filename, "ssd/LeNet-");
	filename[10] = (char) (num_vertices / 1000) + '0';
	filename[11] = (char) (num_vertices % 1000 / 100) + '0';
	filename[12] = (char) (num_vertices % 100 / 10) + '0';
	filename[13] = (char) (num_vertices % 10) + '0';
	strcat(filename, "vertices");
	strcat(filename, "_directed.grf");
	filename[36] = '\0';
	...
	for (vertice = 0; vertice < Source_graph->V; vertice++) {
		...
		fprintf(wif, "\n%d %d %d", Source_graph->memory[vertice], computationalWeight[layer], Source_graph->degree[vertice]);
